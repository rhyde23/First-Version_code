...
def put_standings_in_correct_order(standings_dictionary) :
    return sorted(standings_dictionary, key=lambda x:standings_dictionary[x], reverse=True)

def update_top_scorers(current_data, top_scorers_dict, top_scorers_in_order, user_scorers, user_team, opponent_scorers, opponent_team) :
    max_top_scorers = 3
    top_scorers_added = 0
    ...
    return {key:top_scorers_dict[key] for key in new_dict}, new_dict

current_data['CurrentStandingsInOrder'] = put_standings_in_correct_order(current_data['CurrentStandings'])
        ...
        if match_simming :
            ...
            if currently_simming :
                ...
                if current_minute_count == current_minute_max_count :
                    ...
                    #Accounts for if someone scores in the current_minute
                ...
                if event.type == pygame.MOUSEBUTTONDOWN :
                    if over_sim_button :
                        if stopped_on_matchday :
                            ...
                            current_data['CurrentStandingsInOrder'] = put_standings_in_correct_order(current_data['CurrentStandings'])
                            current_data['TopScorers'], current_data['TopScorersInOrder'] = update_top_scorers(current_data, current_data['TopScorers'], current_data['TopScorersInOrder'], user_scorers, team, opponent_scorers, next_opponent)
                        else :
                            if not simming :
                                simming = True
